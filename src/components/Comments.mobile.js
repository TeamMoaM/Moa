...
function Comments({ id, user, isAuth }) {
  ...
  useEffect(() => {
    const get = async () => {
      const docSnap = query(
        collection(doc(collection(db, "community"), id), "comments")
      );
  ...
  const addCommentLike = async (commentId) => {
    if (isAuth) {
      ...
      if (userLike) {
        ...
        await updateDoc(doc(db, "community", id, "comments", commentId), {
          likeCount: likeCount,
  ...
  const inputPress = async (e, index) => {
    if (e.key == "Enter") {
      var input = document.getElementById("commentAddInput" + index);
      input.value = "";
          ...
          <div className="comment">
              ...
              <div className="postInformation">
                ...
                <div className="postProfile">
                  <div className="postProfile1">
                    <img className="postProfile1Img" src={bronzeMedal} />
                    <h4 id="postAuthorName" className="subhead100">
                      {comment.commentPeople}
                    </h4>
                    <h4 id="postCompanyName" className="body100">
                      {"회사 이름"}
                    </h4>
                  </div>
                  <div className="postProfile1">
                    <TimeCal time={comment.time.seconds} />
                  </div>
                </div>
              </div>
            ...
            <div className="commentLikeAndReplyCommentBox">
              <div className="commentLikeAndReplyComment">
                ...
                >
                  {isAuth &&
                    (!comment.like.includes(user.displayName) ? (
                      <img src={smallHeart} />
                    ) : (
                      <img src={smallHearted} />
                    ))}
                  <h2 className="caption100">공감 {comment.likeCount}개</h2>
                </div>
                {isAuth && (
                  <div
                    className="replyCommentToggleButton"
                    onClick={() => {
                      if (commentInputToggleBool) {
                        setCommentInputToggleList(comment.id);
                        setCommentInputToggleBool(0);
                      } else {
                        setCommentInputToggleBool(1);
                      }
                    }}
                  >
                    <img src={smallComment} />
                    <h2 className="caption100">답글 달기</h2>
                  </div>
                )}
              </div>
            </div>
            ...
            {commentInputToggleList.includes(comment.id) ? (
              <div
                id={"commentInputAndButton" + comment.id}
                className="commentInputAndButton"
              >
                <input
                  onBlur={(e) => {
                    var input3 = document.getElementById(comment.id + "button");
                    input3.style.borderColor = "#E8E8EE";
                    var input1 = document.getElementById(
                      "commentAddInput" + comment.id
                    );
                    input1.value = "";
                  }}
                  onFocus={(e) => {
                    var input = document.getElementById(comment.id + "button");
                    input.style.borderColor = "#2727AD";
                    ...
                    input1.value = "";
                    input1.style.borderRight = "none";
                    input1.style.borderTopRightRadius = "0px";
                    input1.style.borderBottomRightRadius = "0px";
                  }}
                  id={"commentAddInput" + comment.id}
                  onKeyPress={(e) => {
                    inputPress(e, comment.id);
                  }}
