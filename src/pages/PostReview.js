...
function PostReview(props) {
  ...
  useEffect(() => {
    const get = async () => {
      const docSnap = query(
        collection(doc(collection(db, "posts"), roomId), "review")
      );
  ...
  return (
    <div className="postServiceIntro">
      {showReviewNumber == 0 ? (
        <>
          {review &&
            review.map((reviewPost) => {
              return (
                <div
                  onClick={() => {
                    setShowReviewNumber(reviewPost.id);
                    navigate("");
                  }}
                  className="postReviewWrap"
                >
                  <div className="postReviewBox">
                    ...
                    <div className="postReviewContent">
                      <h3
                        ...
                        dangerouslySetInnerHTML={{
                          __html: reviewPost.reviewContent,
                        }}
                      ></h3>
                    </div>
                    <div className="postReviewTimeAndLike">
                      ...
                      <h3 id="postReviewLike" className="body100">
                        추천 {"0"}개
                      </h3>
                    </div>
                  </div>
        ...
        <ReviewDetail
          id={showReviewNumber}
          setShowReviewNumber={setShowReviewNumber}
        />
      )}
    </div>
  );
