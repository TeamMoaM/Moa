...
function ReviewDetail({ id, setShowReviewNumber }) {
  ...
  useEffect(() => {
    getDoc(
      doc(collection(doc(collection(db, "posts"), roomId), "review"), id)
    ).then((docSnap) => {
  ...
  return (
    <div className="reviewDetail">
      <div className="reviewDetailTitleAndInfo">
        <h3 className="title150">{title}</h3>
        <div className="info">{author.length && <UserInfo uid={author} />}</div>
      </div>
      <div
        className="reviewDetailDesc"
