...
function BetaTest_recent({ setList }) {
  ...
  const q = query(
    postsCollectionRef,
    orderBy("id"),
    startAt(pageNumber * onepageNumber + 1),
    limit(onepageNumber)
  );
  useEffect(() => {
    ...
    <div className="BetaTest">
      <div className="functions">
        <div className="BetaTestOrder">
          <Link className="linkLateOrderRecent" to="/BetaTest/recentOrder">
            <h3 className="subhead3">최신순</h3>
          </Link>
          <div className="BetaTestOrderBlockLine">|</div>
          <Link className="linkRecentOrderRecent" to="/BetaTest/lateOrder">
